...
class TelegramParser:
    ...
    async def parse_channel(self, channel_url: str, limit: int = 50) -> List[Dict]:
        """
        Парсит сообщения из Telegram-канала
        
        Args:
            channel_url: URL или username канала (например, @channel_name или https://t.me/channel_name)
        ...
        try:
            ...
            async for message in self.client.iter_messages(
                ...
                if message.text:  # Только текстовые сообщения
                    message_data = {
                        ...
                        'link': f"https://t.me/{entity.username}/{message.id}" if hasattr(entity, 'username') and entity.username else f"https://t.me/c/{entity.id}/{message.id}",
            ...
            return await self.parse_channel(channel_url, limit)
...
async def parse_telegram_channel(channel_url: str, api_id: str, api_hash: str, phone_number: str, limit: int = 50) -> List[Dict]:
    """
